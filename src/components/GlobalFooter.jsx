...
export default function GlobalFooter() {
  return (
    <footer className="relative bg-pearl border-t border-ink/10">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 mb-14">
          <div className="lg:col-span-5">
            <div className="flex items-center gap-3 mb-6">
              ...
              <span className="font-display text-2xl text-ink">{parent.name}</span>
            </div>
            <p className="text-concrete text-sm leading-relaxed max-w-sm">{parent.tagline}.</p>
          </div>

          <div className="lg:col-span-3">
            <div className="text-concrete text-[10px] tracking-[0.25em] uppercase mb-4">Corporate</div>
            <div className="flex flex-col gap-3">
              {PAGES.map((p) => (
                <Link key={p.to} to={p.to} className="text-ink/70 hover:text-azure text-sm transition-colors">
                  {p.label}
                </Link>
              ))}
            </div>
          ...
          <div className="lg:col-span-4">
            <div className="text-concrete text-[10px] tracking-[0.25em] uppercase mb-4">Subsidiaries</div>
            <div className="grid grid-cols-2 gap-x-6 gap-y-3">
              {entities.map((e) => (
                <Link
                  key={e.id}
                  to={`/our-business/${e.slug}`}
                  className="text-ink/70 hover:text-azure text-sm transition-colors"
                >
                  {e.name}
                </Link>
              ))}
